...
export function ColorNamingAnimation() {
  ...
  return (
    <div
      ref={containerRef}
      className="bg-gray-100 dark:bg-neutral-800 border border-neutral-800/5 dark:border-white/5 rounded-2xl my-6 sm:my-12 py-4 sm:py-6 px-8 sm:px-12 relative"
    >
          ...
          <AnimatePresence mode="sync">
            {items.map((item, index) => (
              <motion.div
                key={item.id}
                layout
                initial={item.isOriginal ? false : { opacity: 0, scale: 0.95 }}
